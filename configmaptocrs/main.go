...
func main() {
	sourceFile := flag.String("source", "./config.yaml", "name of the configmap file to convert")
	outputFile := flag.String("output", "./resources.yaml", "name of the file to output generated CRs to")
	flag.Parse()
	log.Printf("MetalLB generator starting. commit: %s branch: %s goversion: %s",
		version.CommitHash(), version.Branch(), version.GoString())

	f, err := os.Create(filepath.Join(inputDirPath, *outputFile))
	if err != nil {
		log.Fatalf("failed to create output file: %s", err)
	}
	defer f.Close()

	err = generate(f, *sourceFile)
	if err != nil {
